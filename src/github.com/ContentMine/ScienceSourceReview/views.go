...
func homeHandler(config *ServerConfig, w http.ResponseWriter, r *http.Request) {

	res, err := getArticleList(config.QueryServiceURL)
	if err != nil {
		panic(err)
	}
...
func articleHandler(config *ServerConfig, w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	id := vars["id"]

	title, annotations, err := getArticleAnnotationList(config.QueryServiceURL, id)
	if err != nil {
		panic(err)
	}
